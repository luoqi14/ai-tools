...
export const api = {
  ...
  async getMeituFunctions(): Promise<MeituFunctionsResponse> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/meitu/functions`);
      const result = await response.json();
  ...
  async processMeituImage(data: MeituProcessingRequest): Promise<{ task_id: string; error?: string }> {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/meitu/process`, {
        method: 'POST',
        body: formData,
      });
  ...
  async getMeituTaskStatus(taskId: string): Promise<MeituProcessingTask | null> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/meitu/status/${taskId}`);
  ...
  async previewMeituProcessing(data: MeituProcessingRequest): Promise<{ preview_image?: string; error?: string }> {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/meitu/preview`, {
        method: 'POST',
        body: formData,
      });

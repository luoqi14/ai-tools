              ...
              <CardContent>
                {functionsData && (
                  ...
                  >
                    {/* 标签页导航 */}
                    <div className="mb-6">
                      <TabsList className="grid w-full grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 bg-gray-100/50 p-2 rounded-lg h-auto !w-full !bg-gray-100/50">
                        {Object.entries(functionsData.parameter_groups).map(
                          ([key, group]) => (
                            <TabsTrigger
                              key={key}
                              value={key}
                              className="flex flex-col items-center justify-center p-3 min-h-[70px] text-xs font-medium transition-all data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:border-purple-200 hover:bg-white/70 cursor-pointer border border-transparent rounded-md relative z-10"
                            >

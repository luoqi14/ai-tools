...
export default function MeituProcessor() {
  ...
  const handleProcess = async () => {
    ...
    const request: MeituProcessingRequest = {
      ...
      // 美颜参数
      whitening_level: brightening,
      smoothing_level: skinSmoothing,
      acne_removal: true,
      // 特效参数
      effect_type: effectStyle,
      intensity: effectStrength,
      // 动漫化参数
      anime_style: effectStyle,
      color_enhancement: true,
      // 增强参数
      denoise_level: intensity,
      sharpen_level: sharpness,
      upscale_factor: 2,
      // 滤镜参数
      filter_type: effectStyle,
      filter_strength: effectStrength,
    };
  ...
  const downloadImage = () => {
    if (task?.result?.processed_image) {
      const link = document.createElement("a");
      link.href = task.result.processed_image;

        ...
        Object.entries(data.parameter_groups).forEach(([groupKey, group]) => {
          Object.entries(group.parameters).forEach(([paramKey, config]) => {
            if (
              ...
              initialParams[paramKey] = data.people_types.map(
                () => config.default_value
              );
            } else {
              initialParams[paramKey] = config.default_value;
            }
          });
        });
    ...
    const poll = async () => {
      try {
        ...
        if (response.status === "completed") {
          setResult(response.result_url);
          setIsPolling(false);
          setPollingProgress(100);
          toast.success("图片处理完成！");
        } else if (response.status === "failed") {
          setIsPolling(false);
          toast.error("图片处理失败: " + response.error);
        } else {
                          ...
                          {Object.entries(group.parameters).map(
                            ([paramKey, config]) => (
                              <ParameterControl
                                ...
                                config={config}
                                currentValue={
                                  parameters[paramKey] || config.default_value
                                }
